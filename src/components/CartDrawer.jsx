...
export default function CartDrawer({ open, items, subtotal, onClose, onRemove, onUpdateQty }) {
  const shipping = subtotal >= FREE_SHIPPING_MIN ? 0 : items.length ? SHIPPING_FLAT : 0;
    ...
    <>
      ...
      >
        <div className="flex h-[68px] items-center justify-between border-b border-cream-dark px-6">
          <h2 className="font-serif text-xl text-ink">Your Bag ({items.length})</h2>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {items.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center">
              <svg className="mb-4 h-12 w-12 text-muted/40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
            ...
            <div className="space-y-5">
              {items.map((item) => (
                <div key={item.lineKey} className="flex gap-4">
                  <img src={item.image} alt={item.name} onError={(e) => { e.target.src = "https://via.placeholder.com/80x100?text=Item"; }} className="h-24 w-20 rounded-xl object-cover" />
                  <div className="flex flex-1 flex-col">
                    <div className="flex items-start justify-between">
                      <div>
                        <h3 className="text-sm font-medium text-ink">{item.name}</h3>
                        <p className="text-xs text-muted">{item.category} · Size {item.size}</p>
                      </div>
                      <button
                        type="button"
                    ...
                    <div className="mt-auto flex items-center justify-between">
                      <div className="flex items-center gap-2 rounded-full border border-cream-dark">
                        <button
                          type="button"
                          onClick={() => onUpdateQty(item.lineKey, item.qty - 1)}
        ...
        {items.length > 0 && (
          <div className="border-t border-cream-dark px-6 py-5">
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="text-sm">{shipping === 0 ? "Free" : formatPrice(shipping)}</span>
            </div>
